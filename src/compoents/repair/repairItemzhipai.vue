<template>
  <div>
    ...
    <div class="errItemFalseSb-box-item">
      <p class="errItemFalseSb-box-item-l">{{list.assignCompletionTime}}</p>
    ...
    <div class="errItemFalseSb-box-item">
      ...
      <textarea v-model="list.checkquestionInfo" readonly class="thing-miaosu" placeholder="请输入巡检描述"></textarea>
    </div>
    <div class="errItemFalseSb-box-item">
...
export default {
  ...
  methods: {
    ...
    // 完成审核提交数据
    async fishShenhe () {
      const { data } = await this.$ajax.post(`repairOrder/assignAudit?sid=${window.sessionStorage.getItem('token')}&status=${this.pass}&type=1`, {
